    ...
    begin
      ...
      if exception_class.methods(false).include?(:type) && exception.instance_variable_get('@data')
        ...
        if Chitti::Errors::GRPCErrorRegistry[package_prefix]
          begin
            encoded_error = Base64.strict_encode64(Chitti::Errors::GRPCErrorRegistry[package_prefix][:ctr].encode(exception.instance_variable_get('@data')))
          rescue Exception => err
            raise err
          end
          user_defined_error = { type: exception_class.name, payload: encoded_error }.to_json
          new_err = GRPC::BadStatus.new(Chitti::Errors::GRPCErrorRegistry[package_prefix][:code], exception.class.type.descriptor.name, grpc_custom_error: user_defined_error)
          raise new_err
        end
      else
        raise exception
